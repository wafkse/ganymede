//! Generic coherent GNU runtime-linker snapshots.
//!
//! Snapshot values retain one sealed GNU ABI parameter from acquisition through module and namespace
//! observation. Width-specific convenience aliases are intentionally absent so the ABI family remains
//! explicit in every public snapshot type.
...
pub mod prelude {
    //! This is the `ganymede-linker-gnu` snapshot prelude.
    //!
    //! It re-exports the generic coherent snapshot records and retry policy without selecting a target
    //! ABI or introducing width-specific aliases.

    pub use super::{Module, Namespace, Rendezvous, RetryPolicy, Snapshot};
}
